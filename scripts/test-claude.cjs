...
class ClaudeCompatTester {
  constructor() {
    this.gateway = config.GATEWAY;
    this.claudeConfig = config.CLAUDE_COMPAT;
  ...
  async makeRequest(prompt, retries = 0) {
    return new Promise((resolve, reject) => {
      const postData = JSON.stringify({
        ...
        max_tokens: this.claudeConfig.MAX_TOKENS,
        messages: [{ role: 'user', content: prompt }]
      });

      const baseUrl = this.gateway.URL_BASE;
      const url = new URL(baseUrl + this.claudeConfig.ENDPOINT);
      ...
      const options = {
        ...
        headers: {
          ...
          'anthropic-version': this.claudeConfig.VERSION,
  ...
  async runTests() {
    console.log('🧠 API Gateway Claude兼容接口测试');
    ...
    console.log(`📋 测试端点: ${this.claudeConfig.ENDPOINT}`);
    console.log(`🎯 使用模型: ${this.claudeConfig.MODEL}`);
    console.log(`⏱️  超时时间: ${this.common.TIMEOUT}ms`);
    console.log(`📊 API版本: ${this.claudeConfig.VERSION}`);
    ...
    for (let i = 0; i < config.TEST_PROMPTS.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          ...
          const content = result.data.content && result.data.content[0] ? 
            result.data.content[0].text : 
            JSON.stringify(result.data);
          console.log(`📝 响应: ${content.substring(0, 100)}...`);
          
          if (result.data.usage) {
            console.log(`💰 Token使用: 输入${result.data.usage.input_tokens} + 输出${result.data.usage.output_tokens} = ${result.data.usage.input_tokens + result.data.usage.output_tokens}`);
          }
        } else {
          failCount++;
...
if (require.main === module) {
  const tester = new ClaudeCompatTester();
  tester.runTests().catch(console.error);
}

module.exports = ClaudeCompatTester;